...
| Path | What it does |
|---|---|
| `xmodel_download/_common.sh` | Shared downloader helpers: conda/`hf` CLI setup, resumable `dl_repo` wrapper (per-flag `--include/--exclude`), post-download `verify_paths` check. Weights land under `${GAGI_ROOT}/xmodels/` (default `/data/datasets/xmodels/`), outside git. |
...
export GAGI_ROOT=/data/datasets        # cluster data root; adapt to your storage
