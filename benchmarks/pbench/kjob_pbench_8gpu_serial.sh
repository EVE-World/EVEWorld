#!/usr/bin/env bash
#SBATCH --job-name=pbench_8gpu_serial
...
mkdir -p "${OUTPUT_ROOT}"
JOB_LOG="${OUTPUT_ROOT}/pbench_8gpu_serial_${CHAIN_NAME}_$(date +%Y%m%d_%H%M%S).log"
touch "${JOB_LOG}"
echo "chain=${CHAIN_NAME} host=$(hostname) models=${MODELS}" | tee -a "${JOB_LOG}"
...
echo "===== $(date '+%F %T') ALL DONE chain=${CHAIN_NAME}: ${MODELS}" | tee -a "${JOB_LOG}"
