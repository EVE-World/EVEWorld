...
if [[ -z "${VIDEO_DIR:-}" ]]; then
  ...
  echo "  VIDEO_DIR=/data/datasets/gr1_dreamgen_eval/dreamgenbench_video_dirs/<run> ./benchmarks/pbench/launch_dreamgenbench_videophy_pa2_kjob.sh" >&2
  exit 1
fi
...
export VIDEOPHY_DIR="${VIDEOPHY_DIR:-/data/datasets/videophy}"
export VIDEOPHY_PYTHON="${VIDEOPHY_PYTHON:-/opt/eveworld/miniconda/envs/giga_models/bin/python}"
