...
DATA_PATH="${DATA_PATH:-${GAGI}/gr1_dreamgen_eval/giga_input/wmb_robotics_it2v.json}"
...
model_dir_for() {
  case "$1" in
    ...
    wmb_*) echo "${GAGI}/wmb_adapt/probes/$1" ;;
    *) echo "" ;;
  esac
}

mkdir -p "${OUTPUT_ROOT}"
JOB_LOG="${OUTPUT_ROOT}/wmb_serial_$(date +%Y%m%d_%H%M%S).log"
touch "${JOB_LOG}"
echo "host=$(hostname) models=${MODELS} seed=${SEED}" | tee -a "${JOB_LOG}"
...
for model in ${MODELS}; do
  ...
  SAVE_DIR="${OUTPUT_ROOT}/${model}"
  SUMMARY_PATH="${SAVE_DIR}/generation_summary.json"
  if [[ -f "${SUMMARY_PATH}" ]]; then
    echo "SKIP ${model}: summary exists" | tee -a "${JOB_LOG}"
    continue
  fi
  mkdir -p "${SAVE_DIR}"
  echo "===== $(date '+%F %T') START ${model}" | tee -a "${JOB_LOG}"
  ...
  echo "===== $(date '+%F %T') DONE ${model}: rc=${rc} mp4=${n_mp4}/50" | tee -a "${JOB_LOG}"
  if [[ "${rc}" != "0" ]]; then
    overall_rc=1
  fi
done
