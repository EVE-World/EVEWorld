...
TRAIN_VENV="${TRAIN_VENV:-/data/datasets/envs/giga_world_train_venv}"
RUNTIME_CONFIG="${RUNTIME_CONFIG:-/data/datasets/wmb_adapt/runtime_configs/t4g_wmb_apre_s600.json}"
LOG_DIR=/data/datasets/wmb_adapt/t4g_wmb_apre
mkdir -p "${LOG_DIR}"
...
export HF_HOME=/data/datasets/.hf_home
export T4G_W_LAT=40
export T4G_WPIX=640
...
N_AUG=$(ls /data/datasets/wmb_adapt/aug_assets_v1/*.npz 2>/dev/null | wc -l)
echo "aug assets present: ${N_AUG}/500"
if [[ "${N_AUG}" -lt "500" ]]; then
  N_SHARDS=16 /opt/eveworld/miniconda/envs/giga_world1/bin/python \
    eveworld/agibot/w6_dispatch.py
  echo "aug fill rc=$? now=$(ls /data/datasets/wmb_adapt/aug_assets_v1/*.npz 2>/dev/null | wc -l)/500"
fi
