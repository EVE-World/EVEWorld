...
`EVEWORLD_ROOT` must point to this repository root. All `kjob_*` / `launch_*` scripts accept `KEY=VALUE` overrides; data roots default to the `GAGI_ROOT` convention (`/data/datasets`, see `eveworld/common/env.sh`).
