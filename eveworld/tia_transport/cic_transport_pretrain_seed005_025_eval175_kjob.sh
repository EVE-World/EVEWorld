#!/usr/bin/env bash
#SBATCH --job-name=eve_pretrain_s005_s025
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${CAMPAIGN_ROOT}/eval175_pretrain_raw_s000_seed005_seed025}"
PROBE_ROOT="${PROBE_ROOT:-${CAMPAIGN_ROOT}/eval175_pretrain_raw_s000_seed005_seed025_probes}"
...
[[ "$(realpath -m "${OUTPUT_ROOT}")" == "${CAMPAIGN_ROOT}/eval175_pretrain_raw_s000_seed005_seed025" ]] || {
  echo "Refusing unexpected OUTPUT_ROOT=${OUTPUT_ROOT}" >&2
  exit 2
}
[[ "$(realpath -m "${PROBE_ROOT}")" == "${CAMPAIGN_ROOT}/eval175_pretrain_raw_s000_seed005_seed025_probes" ]] || {
  echo "Refusing unexpected PROBE_ROOT=${PROBE_ROOT}" >&2
  exit 2
}
[[ "$(realpath -m "${MANIFEST_ROOT}")" == "${CAMPAIGN_ROOT}/eval175_pretrain_raw_s000_seed005_seed025_eval" ]] || {
...
"${PYTHON}" eveworld/evaluation/eval175_seed_sharded_dispatch.py \
  ...
  --chain-name "pretrain_seed005_seed025" \
  --seeds "${seeds[@]}" \
  --gpu-count 8 \
  --python "${PYTHON}" \
...
echo "[campaign] Pretrain seed005/025 generated and audited"
