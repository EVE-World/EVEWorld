...
CONDA_SH="/opt/eveworld/miniconda/etc/profile.d/conda.sh"
CONDA_ENV="giga_models"
ROOT="/data/datasets/eve_v2_outputs/eve_cic_transport_v1"
MANIFEST_ROOT="${ROOT}/eval175_references_pretrain_s000_sft_s150_seed049_seed062_eval"
OUTPUT_ROOT="${ROOT}/qwen_references_pretrain_s000_sft_s150_seed049_seed062_thinking_off"
...
while ! {
  ...
  for model in "${models[@]}"; do
    jq -e '.ready == true and .manifest_count == 252 and (.errors | length) == 0' \
...
if [[ "${result}" == "0" ]]; then
  python eveworld/tia_transport/cic_transport_reference_seed049_062_qwen_results.py \
