...
OUTPUT_ROOT="${OUTPUT_ROOT:-${CAMPAIGN_ROOT}/eval175_references_pretrain_s000_sft_s200_seed066}"
PROBE_ROOT="${PROBE_ROOT:-${CAMPAIGN_ROOT}/eval175_references_pretrain_s000_sft_s200_seed066_probes}"
MANIFEST_ROOT="${MANIFEST_ROOT:-${CAMPAIGN_ROOT}/eval175_references_pretrain_s000_sft_s200_seed066_eval}"
PRETRAIN_WEIGHT_DIR="${GAGI}/giga_world_0_video_pretrain/transformer"
SFT_WEIGHT_DIR="${GAGI}/eve_v2_outputs/round0_fullft/experiments_round0/models/checkpoint_epoch_200_step_200/transformer"
WORKER="${EVEWORLD_ROOT}/eveworld/evaluation/eval175_multiseed_worker.py"
RESUME="${RESUME:-0}"
models=(pretrain_raw_s000 standard_sft_raw_s200)
seeds=(66)

[[ "$(realpath -m "${OUTPUT_ROOT}")" == "${CAMPAIGN_ROOT}/eval175_references_pretrain_s000_sft_s200_seed066" ]] || {
  echo "Refusing unexpected OUTPUT_ROOT=${OUTPUT_ROOT}" >&2
  exit 2
}
[[ "$(realpath -m "${PROBE_ROOT}")" == "${CAMPAIGN_ROOT}/eval175_references_pretrain_s000_sft_s200_seed066_probes" ]] || {
  echo "Refusing unexpected PROBE_ROOT=${PROBE_ROOT}" >&2
  exit 2
}
[[ "$(realpath -m "${MANIFEST_ROOT}")" == "${CAMPAIGN_ROOT}/eval175_references_pretrain_s000_sft_s200_seed066_eval" ]] || {
...
for model in "${models[@]}"; do
  case "${model}" in
    ...
    standard_sft_raw_s200) weight_dir="${SFT_WEIGHT_DIR}" ;;
  ...
  "${PYTHON}" eveworld/evaluation/eval175_seed_sharded_dispatch.py \
    ...
    --chain-name "reference_${model}_seed066" \
    --seeds "${seeds[@]}" \
    --gpu-count 8 \
    --python "${PYTHON}" \
...
echo "[campaign] Pretrain s000 and SFT s200 seed066 generated and audited"
