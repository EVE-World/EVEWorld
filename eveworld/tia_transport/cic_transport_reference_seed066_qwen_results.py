#!/usr/bin/env python3
"""Audit Pretrain/SFT seed066 Qwen results and compare Transport s200."""
...
DEFAULT_TRANSPORT_ROOT = Path(
    "/data/datasets/eve_v2_outputs/eve_cic_transport_v1/"
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        split_values = {split: [] for split in SPLITS}
        for repeat in range(1, 4):
            run_name = f"s200_all70_repeat{repeat:02d}"
            run_dir = args.transport_root / run_name
            config = json.loads(
                (run_dir / f"{run_name}_run_config.json").read_text(encoding="utf-8")
            )
            if config.get("disable_thinking") is not True:
                raise ValueError(f"{run_name}: Transport thinking is not disabled")
            if config["qwen_base"] not in endpoints or config["qwen_model"] not in judges:
                raise ValueError(f"{run_name}: Transport judge protocol drift")
            csv_path = run_dir / f"{run_name}_qwen_if.csv"
            with csv_path.open("r", encoding="utf-8", newline="") as handle:
                rows = [
                    row
    ...
    for seed in SEEDS:
        ...
        sft = report_aggregate["standard_sft_raw_s200"][seed_name]["mean_percent"]
    ...
    report = {
        "schema": "eve-reference-seed066-qwen-three-repeat-v1",
        "protocol": {
        ...
        "transport_s200_aggregate": transport_aggregate,
