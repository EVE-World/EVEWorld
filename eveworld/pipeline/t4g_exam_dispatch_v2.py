#!/usr/bin/env python3
"""T4G-EXAM v2 分发+合并: 枚举核心臂 eval175 视频 -> 8 卡分片 v2 体检 -> 修正病图结果。"""
import glob
import json
import os
import subprocess
...
ARMS = ['pretrain', 'gr1_2b', 'round0', 't4g_wmapA_pre_seed42_s250',
        't4g_wmapA_pre_s250', 't4g_wmapA_pre_cleanv4_u3_s300', 't4g_wmapA_s100']
...
def build_list(out_dir, arms):
    ...
    with open(fp, 'w') as f:
        for arm in arms:
            arm_root = f'{GEN}/{arm}'
            if SEED:
                arm_root = f'{arm_root}/seed{int(SEED):03d}'
            for sp in SPLITS:
                prompts = [it['prompt'] for it in json.load(open(f'{INP}/eval175_{sp}.json'))]
                vdir = f'{arm_root}/{sp}/generated_only'
                ...
                for i, p in enumerate(prompts):
                    if i in vids:
                        ...
                        n += 1
    print(f'[exam_v2] list: {n} videos -> {fp}')
    return fp
...
def merge(out_dir):
    ...
    print(f'\n===== T4G-EXAM v2 恒存性体检 (GDINO v2 物理去噪确定性数病例) =====')
    print(f'{"臂":32s} {"n":>4s} {"不可检":>5s} | {"DUP复制率":>8s} {"VANISH消失率":>10s} {"平均超库存峰值":>8s}')
    summary = {}
    for arm in arms:
        ...
        summary[arm] = dict(n=len(rs), undetectable=und, dup_rate=dup, vanish_rate=van,
                            mean_excess_peak=mx)
        print(f'{arm:32s} {len(rs):>4d} {und:>5d} | {dup:>8.3f} {van:>10.3f} {mx:>8.2f}')
    json.dump(dict(summary=summary, records=recs), open(f'{out_dir}/exam_v2_summary.json', 'w'))
    print(f'MERGED -> {out_dir}/exam_v2_summary.json')
...
def main():
    ...
    for g in range(ngpu):
        ...
        procs.append(subprocess.Popen(
            [tp, os.path.join(here, 't4g_exam_v2.py'), '--list-file', lf,
