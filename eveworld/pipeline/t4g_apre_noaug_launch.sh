...
OUT_ROOT="${GAGI}/eve_v2_outputs/t4g_joint_wmapA_pre_noaug"
RUN_NAME="${RUN_NAME:-t4g_wmapA_pre_noaug_s300}"
BASE_CONFIG_MODULE="eveworld.pipeline.t4g_apre_noaug_config"
MAX_STEPS="${MAX_STEPS:-300}"
CHECKPOINT_INTERVAL="${CHECKPOINT_INTERVAL:-50}"
CHECKPOINT_TOTAL_LIMIT="${CHECKPOINT_TOTAL_LIMIT:-8}"
SEED="6666"
PYTHON="/opt/eveworld/miniconda/envs/giga_models/bin/python"
...
run_check() {
  ...
  "${PYTHON}" -m py_compile \
    eveworld/pipeline/t4g_apre_noaug_config.py \
