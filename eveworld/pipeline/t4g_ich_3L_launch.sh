#!/usr/bin/env bash
# ICH 训练 · 臂 B (三层 block10+12+16) —— kjob 提交入口 (71 号 B1 G3, 50 步探针)。
# 与臂 A (t4g_ich_1L_launch.sh) 唯一差异 = config (t4g_ich_blocks)。
...
PACKED="/data/datasets/gr1_finetune_data/packed_data"
OUT_ROOT="/data/datasets/eve_v2_outputs/t4g_ich_3L"
...
RUN_NAME="${RUN_NAME:-t4g_ich_3L_s50}"
...
case "${1:-}" in
  submit) run_submit ;;
  *) cat <<EOF
=========== ICH 臂 B (block10+12+16 三层) 50 步训练 (未提交) ===========
