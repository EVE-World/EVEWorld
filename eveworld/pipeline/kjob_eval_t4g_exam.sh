#!/usr/bin/env bash
#SBATCH --job-name=eve_eval_t4g_exam
...
OUT_DIR="${OUT_DIR:-/data/datasets/eve_v2_outputs/track4gen_probe/exam175_v2}"
...
echo "== T4G-EXAM 恒存性体检分发: out=${OUT_DIR} =="
nvidia-smi || true
...
"${TRAIN_PYTHON}" eveworld/pipeline/t4g_exam_dispatch.py 8 "${OUT_DIR}" "${TRAIN_PYTHON}"
exit "$?"
