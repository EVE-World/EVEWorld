...
Launchers accept trailing `KEY=VALUE` arguments and export them as environment variables. `kjob_*.sh` / `launch_*.sh` are SLURM-style wrappers (SBATCH headers, pod polling) whose absolute paths — `/data/datasets/...`, venv locations, scheduler integration — **must be adapted to your cluster**.
