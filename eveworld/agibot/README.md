...
- **Paths are constants, not flags.** Most scripts here hardcode module-level roots under the internal `GAGI_ROOT` layout (`/data/datasets`, see [`eveworld/common/env.sh`](../common/env.sh)); adapt them (or keep the same layout) before running. Repo-level wrappers use `EVEWORLD_ROOT` for the repository root.
