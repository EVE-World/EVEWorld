...
case "$POOL" in
  longpool)
    ROOT=/data/datasets/eve_v2_outputs/longpool_f125/gr1_sft_ema
    TAG=longpool_f125 ; SUFFIX=f125 ;;
  round0)
    ROOT=/data/datasets/eve_v2_outputs/pool_round0_f93
    TAG=pool_round0_f93 ; SUFFIX=f93 ;;
  *) echo "未知池: $POOL (longpool|round0)"; exit 1 ;;
esac
OUT=/data/datasets/eve_v2_outputs/scores/$TAG
mkdir -p "$OUT"
