...
set -eu
source /opt/eveworld/miniconda/etc/profile.d/conda.sh && conda activate giga_models
REPO=giga-world-0
TAG="${1:?TAG}"
ROOT=/data/datasets/eve_v2_outputs/probe/${TAG}
OUT=/data/datasets/eve_v2_outputs/scores/probe_${TAG}
...
def load(d):
    ...
    return m
probe = load(f'/data/datasets/eve_v2_outputs/scores/probe_{TAG}')
base  = load('/data/datasets/eve_v2_outputs/scores/pool_round0_f93')
...
json.dump({'tag':TAG,'n':n,'probe_mean':mean(p_all),'round0_mean':mean(b_all),'delta':d,
           'wins':wins,'ties':ties}, open(f'/data/datasets/eve_v2_outputs/scores/probe_{TAG}/verdict.json','w'), indent=1)
EOF
