...
set -euo pipefail
REPO=giga-world-0
PRETRAIN=/data/datasets/giga_world_0_video_pretrain
EMA_CKPT=/data/datasets/giga_world_0_outputs/gr1_finetune/experiments_200_clean/models/checkpoint_epoch_100_step_200/transformer_ema
ARM="${ARM:-gr1_sft_ema}"
OUT_BASE=/data/datasets/eve_v2_outputs/longpool_f125

if [[ "$ARM" == "gr1_sft_ema" ]]; then
  # bestofn 链路写死 MODEL_DIR/{transformer,text_encoder,vae} 结构, 用软链目录适配 EMA ckpt
  MODEL_DIR=/data/datasets/eve_v2_outputs/anchor_models/gr1_fullft_ema
  mkdir -p "$MODEL_DIR"
...
export RL_DIR="${RL_DIR:-/opt/eveworld/new_rl/rl}"
