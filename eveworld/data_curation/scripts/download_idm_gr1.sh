...
DEST="${DEST:-/data/datasets/idm_gr1_probe}"
...
for cand in \
  "${HF_BIN_OVERRIDE:-}" \
  "/opt/eveworld/miniconda/envs/giga_models/bin/hf" \
  "/opt/eveworld/miniconda/envs/giga_models/bin/huggingface-cli" \
...
GM_PY="/opt/eveworld/miniconda/envs/giga_models/bin/python"
if [[ -z "$HF_BIN" ]]; then
