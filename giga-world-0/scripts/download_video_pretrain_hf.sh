...
ROOT_DIR="${ROOT_DIR:-/data/datasets}"
BASE_DIR="${BASE_DIR:-${ROOT_DIR}/giga_world_0_video_pretrain}"
PYTHON_BIN="${PYTHON_BIN:-python}"
MAX_WORKERS="${MAX_WORKERS:-8}"
AUTO_INSTALL_DEPS="${AUTO_INSTALL_DEPS:-1}"

GIGAWORLD_REPO="open-gigaai/GigaWorld-0-Video-Pretrain-2b"
...
main() {
  ...
  download_component "GigaWorld-0 transformer" \
    "${GIGAWORLD_REPO}" \
    --include "transformer/*" \
    --local-dir "${BASE_DIR}"

  download_component "T5-11B text encoder" \
    "${TEXT_ENCODER_REPO}" \
    --local-dir "${BASE_DIR}/text_encoder"

  download_component "Wan VAE" \
    "${VAE_REPO}" \
    --include "vae/*" \
    --local-dir "${BASE_DIR}"
